...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ---- 标定状态记录 ----
        self.calibration_status = {i: False for i in range(8)}  # 默认未标定
    ...
    def init_ui(self):
        ...
        self.z_offset_edit = QLineEdit("50")
    ...
    def on_vision_pallet(self):
        ...
        def vision_pallet_thread():
            ...
            while self.vision_pallet_running and not self.vision_pallet_stop:
                if self.current_step >= len(self.pallet_points):
                ...
                self.vm_thread.data_received.connect(on_data)
                self.vm_thread.trigger_and_get()
                if not event.wait(self.vm_thread.timeout + 1.0):
                    self.append_log("[视觉码垛] 拍照超时，等待后重试...")
                    self.vm_thread.data_received.disconnect(on_data)
                    time.sleep(0.5)
                    continue
                self.vm_thread.data_received.disconnect(on_data)
                ...
                if not received_points:
                    self.append_log("[视觉码垛] 视觉数据为空（无工件），等待后重试...")
                    time.sleep(0.5)
                    continue
                ...
                # 计算剩余码垛点数
                remaining = len(self.pallet_points) - self.current_step
                ...
                for idx, pixel in enumerate(received_points):
                    ...
                    # 视觉转换（动态转换）
                    trig_pose = self.photo_point.pose
                    trig_point = x5.Point(pose=trig_pose, uf=self.photo_point.uf,
                                          tf=self.photo_point.tf, cfg=self.photo_point.cfg)
                    ...
                    try:
                        ...
                        continue
                    self.append_log(f"[视觉码垛] 转换后机器人坐标: X={result_pose.x:.2f}, Y={result_pose.y:.2f}, C={result_pose.c:.2f}")

                    # 移动到抓取点
                    grab_pose = x5.Pose(result_pose.x, result_pose.y, self.photo_point.pose.z,
                                        self.photo_point.pose.a, self.photo_point.pose.b,
                                        result_pose.c, 0, 0, 0)
                    self.append_log("[视觉码垛] 移动到抓取点")
                    self.robot_worker.movl_to_pose(grab_pose, self.photo_point.uf,
                                                   self.photo_point.tf, self.photo_point.cfg)
                    time.sleep(0.3)
                    ...
                    move_type = self.move_type_combo.currentText()
                    if move_type == "MOVJ":
                        self.robot_worker.movj_to_pose(approach_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)
                        self.robot_worker.movj_to_pose(target_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)
                        self.robot_worker.movj_to_pose(depart_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)
                    else:
                        self.robot_worker.movl_to_pose(approach_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)
                        self.robot_worker.movl_to_pose(target_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)
                        self.robot_worker.movl_to_pose(depart_pose, self.pallet_uf,
                                                       self.pallet_tf, self.pallet_cfg)

                    self.current_step += 1
                    QMetaObject.invokeMethod(self, "update_progress", Qt.QueuedConnection,
                                             Q_ARG(int, self.current_step),
                                             Q_ARG(int, len(self.pallet_points)),
