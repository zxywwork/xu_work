...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - 动态J2标定 (带UF/TF切换)")
    ...
    def init_ui(self):
        ...
        robot_layout = QVBoxLayout()  # 改为垂直布局，以便容纳多行
    ...
    def on_apply_config(self):
        ...
        try:
            ...
            mount_type = 2  # 动态J2
            uf_index = self.vm_uf_combo.currentIndex()    # 标定使用的UF
            tf_index = self.vm_tf_combo.currentIndex()    # 标定使用的TF（虚拟工具）
            invert_c = self.vm_c_invert_check.isChecked()

            self.append_log(f"[配置] 视觉工艺: {vision_idx}")
            self.append_log(f"[配置] 视觉坐标系: UF{uf_index}")
            self.append_log(f"[配置] 工具坐标系: TF{tf_index}")
            ...
            self.append_log("[配置] 正在写入 Vision 配置和标定参数...")
            ...
            self.append_log("✅ Vision配置和标定参数全部写入成功！")
    ...
    def on_start_calib(self):
        ...
        self.append_log("[标定] 请确保标定板在相机视野内，且视觉软件已运行")
        self.append_log("[标定] 机器人将自动移动J2轴进行标定，请观察机器人运动")
        self.append_log("="*60)
    ...
    def on_calib_result(self, flag, errors, transformation):
        if flag == 1:
            ...
            if errors[1] == 0.0 and errors[3] == 0.0:
                self.append_log("[警告] Y方向误差为0，可能标定数据异常，请检查视觉数据是否正确包含Y坐标")
            vision_idx = int(self.vision_index_combo.currentText().split()[-1])
            try:
