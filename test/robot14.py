...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - 完整版")
        self.setGeometry(50, 50, 1500, 1300)   # 扩大窗口
        ...
        self._updating_table = False   # 防止编辑时循环触发
    ...
    def init_ui(self):
        ...
        # 新增Z基准偏移
        pallet_inner.addWidget(QLabel("Z基准偏移:"), 1, 8)
        self.z_offset_edit = QLineEdit("0")
        self.z_offset_edit.setFixedWidth(50)
        pallet_inner.addWidget(self.z_offset_edit, 1, 9)
        ...
        # 连接单元格修改信号
        self.point_table.cellChanged.connect(self.on_point_cell_changed)
        pallet_inner.addWidget(self.point_table, 7, 0, 1, 7)

        # 进度显示
        pallet_inner.addWidget(QLabel("进度:"), 8, 0)
        self.progress_label = QLabel("0 / 0")
        pallet_inner.addWidget(self.progress_label, 8, 1)
        pallet_inner.addWidget(QLabel("当前目标:"), 8, 2)
        self.current_target_label = QLabel("--")
        pallet_inner.addWidget(self.current_target_label, 8, 3, 1, 4)

        # 提示
        pallet_inner.addWidget(QLabel("提示: 记录P1,P2,P3 → 设置参数 → 生成点 → 选择点或连续运行"), 9, 0, 1, 7)
    ...
    def on_generate_pallet_points(self):
        ...
        self.append_log(f"[码垛] 成功生成 {self.total_steps} 个目标点")

    def on_point_cell_changed(self, row, column):
        """当表格单元格被修改时，更新 self.pallet_points"""
        if self._updating_table or row >= len(self.pallet_points):
            return
        try:
            new_val = float(self.point_table.item(row, column).text())
        except (ValueError, TypeError):
            self.append_log(f"[错误] 单元格数据无效，请输入数字")
            # 恢复原值
            self._updating_table = True
            pose = self.pallet_points[row]
            val_map = {1: pose.x, 2: pose.y, 3: pose.z, 4: pose.c}
            self.point_table.item(row, column).setText(f"{val_map.get(column, 0):.2f}")
            self._updating_table = False
            return
        ...
        if column == 1:
            ...
            pose.c = new_val
        self.append_log(f"[码垛] 点{row+1} 已更新 (列{column}) = {new_val:.2f}")
