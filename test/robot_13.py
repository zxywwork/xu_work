...
# ============================================================
# 工作线程2：机器人 API 调用（移除所有Pallet相关Lua，改为直接运动）
# ============================================================
class RobotWorker(QThread):
    ...
    def movl_to(self, point):
        with self.cmd_lock:
            self.cmd_queue.append(("movl", point))
    ...
    def move_to_pose(self, pose, uf, tf, cfg):
        """直接运动到指定的Pose（使用当前UF/TF）"""
        with self.cmd_lock:
            self.cmd_queue.append(("move_to_pose", pose, uf, tf, cfg))
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                elif cmd[0] == "move_to_pose":
                    self._do_move_to_pose(cmd[1], cmd[2], cmd[3], cmd[4])
                else:
    ...
    def _do_movl(self, point):
        if self.handle is None:
            return
        try:
            x5.movl(self.handle, point)
            self.log_signal.emit(f"[运动] movl 到 ({point.pose.x:.1f}, {point.pose.y:.1f}, {point.pose.z:.1f})")
        except Exception as e:
            self.log_signal.emit(f"[运动] 失败: {str(e)}")
    ...
    def _do_move_to_pose(self, pose, uf, tf, cfg):
        if self.handle is None:
            self.log_signal.emit("[移动] 机器人未连接")
            return
        try:
            point = x5.Point(pose=pose, uf=uf, tf=tf, cfg=cfg)
            x5.movl(self.handle, point)
            self.log_signal.emit(f"[移动] 到 ({pose.x:.1f}, {pose.y:.1f}, {pose.z:.1f})")
        except Exception as e:
            self.log_signal.emit(f"[移动] 失败: {str(e)}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - 手动码垛工艺")
        self.setGeometry(50, 50, 1400, 1200)
        ...
        # ---- 码垛相关变量 ----
        self.pallet_points = []          # 所有目标点 (Pose)
        self.current_step = 0            # 当前已移动到第几个点
        self.total_steps = 0
        self.p1_point = None
        self.p2_point = None
        self.p3_point = None
        ...
        self.approach_offset = 50.0      # 趋近点Z偏移
        self.depart_offset = 50.0        # 离开点Z偏移
    ...
    def init_ui(self):
        ...
        self.movl_btn = QPushButton("执行 movl (到300,0,100)")
        self.movl_btn.clicked.connect(self.on_movl_test)
        ...
        # 第一行：记录参考点
        pallet_inner.addWidget(QLabel("参考点 P1 (PR10):"), 0, 0)
        ...
        pallet_inner.addWidget(self.p1_label, 0, 2)
        pallet_inner.addWidget(QLabel("行上点 P2 (PR11):"), 0, 3)
        self.p2_record_btn = QPushButton("记录P2")
        self.p2_record_btn.clicked.connect(lambda: self.record_pallet_point(11, "P2"))
        pallet_inner.addWidget(self.p2_record_btn, 0, 4)
        self.p2_label = QLabel("未记录")
        pallet_inner.addWidget(self.p2_label, 0, 5)
        pallet_inner.addWidget(QLabel("列上点 P3 (PR12):"), 0, 6)
        self.p3_record_btn = QPushButton("记录P3")
        self.p3_record_btn.clicked.connect(lambda: self.record_pallet_point(12, "P3"))
        pallet_inner.addWidget(self.p3_record_btn, 0, 7)
        self.p3_label = QLabel("未记录")
        pallet_inner.addWidget(self.p3_label, 0, 8)

        # 第二行：行列层及间距模式
        pallet_inner.addWidget(QLabel("行数:"), 1, 0)
        self.row_edit = QLineEdit("3")
        self.row_edit.setFixedWidth(50)
        pallet_inner.addWidget(self.row_edit, 1, 1)
        pallet_inner.addWidget(QLabel("列数:"), 1, 2)
        self.col_edit = QLineEdit("4")
        self.col_edit.setFixedWidth(50)
        pallet_inner.addWidget(self.col_edit, 1, 3)
        pallet_inner.addWidget(QLabel("层数:"), 1, 4)
        self.layer_edit = QLineEdit("4")
        self.layer_edit.setFixedWidth(50)
        pallet_inner.addWidget(self.layer_edit, 1, 5)
        pallet_inner.addWidget(QLabel("层高(mm):"), 1, 6)
        self.height_edit = QLineEdit("20")
        self.height_edit.setFixedWidth(50)
        pallet_inner.addWidget(self.height_edit, 1, 7)

        # 间距模式
        pallet_inner.addWidget(QLabel("间距模式:"), 2, 0)
        ...
        pallet_inner.addWidget(self.spacing_mode_combo, 2, 1)

        # 行距、列距（默认隐藏，仅在“设定间距”时显示）
        ...
        pallet_inner.addWidget(self.row_dist_label, 2, 2)
        pallet_inner.addWidget(self.row_dist_edit, 2, 3)
        pallet_inner.addWidget(self.col_dist_label, 2, 4)
        pallet_inner.addWidget(self.col_dist_edit, 2, 5)
        # 默认隐藏
        ...
        # 趋近/离开偏移
        pallet_inner.addWidget(QLabel("趋近偏移Z:"), 3, 0)
        self.approach_offset_edit = QLineEdit("50")
        self.approach_offset_edit.setFixedWidth(60)
        pallet_inner.addWidget(self.approach_offset_edit, 3, 1)
        pallet_inner.addWidget(QLabel("离开偏移Z:"), 3, 2)
        self.depart_offset_edit = QLineEdit("50")
        self.depart_offset_edit.setFixedWidth(60)
        pallet_inner.addWidget(self.depart_offset_edit, 3, 3)

        # 安全点
        pallet_inner.addWidget(QLabel("安全点 (PR20):"), 4, 0)
        self.safe_point_label = QLabel("未记录")
        pallet_inner.addWidget(self.safe_point_label, 4, 1, 1, 2)
        self.safe_record_btn = QPushButton("记录安全点")
        self.safe_record_btn.clicked.connect(self.record_safe_point)
        pallet_inner.addWidget(self.safe_record_btn, 4, 3)
        self.safe_move_btn = QPushButton("移动到安全点")
        self.safe_move_btn.clicked.connect(self.move_to_safe_point)
        pallet_inner.addWidget(self.safe_move_btn, 4, 4)

        # 操作按钮
        self.generate_btn = QPushButton("生成码垛点")
        self.generate_btn.clicked.connect(self.on_generate_pallet_points)
        self.move_next_btn = QPushButton("移动到下一个点")
        self.move_next_btn.clicked.connect(self.on_move_to_next)
        self.reset_pallet_btn = QPushButton("重置码垛")
        self.reset_pallet_btn.clicked.connect(self.on_reset_pallet)
        pallet_inner.addWidget(self.generate_btn, 5, 0, 1, 2)
        pallet_inner.addWidget(self.move_next_btn, 5, 2, 1, 2)
        pallet_inner.addWidget(self.reset_pallet_btn, 5, 4, 1, 2)

        # 进度显示
        pallet_inner.addWidget(QLabel("进度:"), 6, 0)
        self.progress_label = QLabel("0 / 0")
        pallet_inner.addWidget(self.progress_label, 6, 1)
        pallet_inner.addWidget(QLabel("当前目标:"), 6, 2)
        self.current_target_label = QLabel("--")
        pallet_inner.addWidget(self.current_target_label, 6, 3, 1, 3)

        # 提示
        pallet_inner.addWidget(QLabel("提示: 记录P1,P2,P3后，设置参数，点击'生成码垛点'，然后依次点击'移动到下一个点'"), 7, 0, 1, 8)
    ...
    def _set_robot_controls_enabled(self, enabled):
        for w in [self.movj_btn, self.movl_btn, self.emergency_btn,
                  ...
                  self.generate_btn, self.move_next_btn, self.reset_pallet_btn]:
    ...
    def on_movl_test(self):
        # 测试movl到固定点
        pose = x5.Pose(300, 0, 100, 0, 0, 0, 0, 0, 0)
        point = x5.Point(pose=pose, uf=0, tf=0, cfg=(0, 0, 0, 1))
        self.robot_worker.movl_to(point)
    ...
    def move_to_safe_point(self):
        ...
        if self.safe_point is None:
            ...
            return
        self.robot_worker.move_to_pose(self.safe_point.pose, self.safe_point.uf, self.safe_point.tf, self.safe_point.cfg)

    def on_generate_pallet_points(self):
        """根据P1,P2,P3和参数生成所有码垛目标点"""
        if not self.is_robot_connected:
            self.append_log("[错误] 请先连接机器人")
            return
        # 检查是否已记录P1,P2,P3
        ...
        try:
            ...
            spacing_mode = self.spacing_mode_combo.currentIndex()  # 0=等分, 1=设定间距
            if spacing_mode == 0:
                row_dist = None
                col_dist = None
            else:
        ...
        if spacing_mode == 0:
            # 等分模式：根据行列数平分向量
            row_step = (row_vec[0] / (row - 1) if row > 1 else 0,
                        row_vec[1] / (row - 1) if row > 1 else 0,
                        row_vec[2] / (row - 1) if row > 1 else 0)
            col_step = (col_vec[0] / (col - 1) if col > 1 else 0,
                        col_vec[1] / (col - 1) if col > 1 else 0,
                        col_vec[2] / (col - 1) if col > 1 else 0)
        else:
            # 设定间距模式：根据用户输入的行距和列距，以及行方向/列方向的单位向量来计算步长
            # 先计算方向向量的长度
            ...
            if row_len == 0 or col_len == 0:
                self.append_log("[错误] P2或P3与P1重合，请重新记录")
                return
            # 单位向量
        ...
        for k in range(layer):
            ...
            for i in range(row):
                for j in range(col):
                    ...
                    z = p1.z + i * row_step[2] + j * col_step[2] + z_offset
                    # 角度保持与P1一致（或可根据需求调整）
        ...
        self.append_log(f"[码垛] 成功生成 {self.total_steps} 个目标点 (行{row}列{col}层{layer})")

    def on_move_to_next(self):
        """移动到下一个码垛目标点（包含趋近和离开动作）"""
        if not self.is_robot_connected:
            self.append_log("[错误] 请先连接机器人")
            return
        ...
        if self.current_step >= self.total_steps:
            self.append_log("[完成] 所有码垛点已执行完毕")
            return

        target_pose = self.pallet_points[self.current_step]
        # 获取当前使用的UF/TF（从P1记录时保存的）
        uf = self.pallet_uf
        tf = self.pallet_tf
        cfg = self.pallet_cfg
        ...
        # 执行运动序列：趋近点 -> 目标点 -> 离开点
        self.append_log(f"[码垛] 移动到第 {self.current_step+1}/{self.total_steps} 个点")
        self.robot_worker.move_to_pose(approach_pose, uf, tf, cfg)
        time.sleep(0.2)  # 简单等待，实际可用 wait_move_done，但这里为了简化
        self.robot_worker.move_to_pose(target_pose, uf, tf, cfg)
        time.sleep(0.2)
        self.robot_worker.move_to_pose(depart_pose, uf, tf, cfg)
