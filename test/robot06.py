...
class RobotWorker(QThread):
    ...
    def set_uf(self, uf_index, pose):
        """设置用户坐标系数据"""
        if self.handle is None:
            return False, "机器人未连接"
        try:
            x5.set_uf(self.handle, uf_index, pose)
            return True, "设置成功"
        except Exception as e:
            return False, str(e)
    ...
    def run_auto_calib(self, vision_idx):
        ...
        try:
            ...
            if flag == 1:
                ...
                self.log_signal.emit("[标定] ⚠️ 标定完成但误差超标 (>5mm)")
            elif flag == -2:
                ...
                self.log_signal.emit("  3. ATTR 值不正确 (需要 0/1/2/3)")
                self.log_signal.emit("  4. 触发方式设置错误")
            else:
...
class MainWindow(QMainWindow):
    ...
    def on_vm_data_received(self, points):
        ...
        if 0 in attrs and 1 in attrs and 2 in attrs and 3 in attrs:
            self.append_log("[VM] ✅ ATTR 包含 0,1,2,3 (圆形/三角形/四边形/五边形) 识别正确")
        else:
            self.append_log("[VM] ⚠️ ATTR 缺少 0/1/2/3，标定可能失败！")
    ...
    def on_apply_config(self):
        ...
        try:
            ...
            self.append_log(f"[配置] 接收格式: {self.vm_format_combo.currentText()} (需要格式1带ATTR)")
    ...
    def on_start_calib(self):
        ...
        if not self.latest_vm_points:
            ...
            self.append_log("[建议] 先点击「📷 触发拍照」确认数据正确")
        else:
            ...
            if 0 not in attrs or 1 not in attrs or 2 not in attrs or 3 not in attrs:
                ...
                self.append_log(f"[警告] 当前 ATTR: {sorted(attrs)}，需要包含 0,1,2,3")
    ...
    def _do_start_calib(self):
        ...
        self.append_log("  3. Vision Master 已连接并返回正确的 ATTR (0/1/2/3)")
        self.append_log("=" * 60)
        ...
        if flag == 1:
            ...
            self.append_log("  4. 触发方式是否设置为「网络触发」")
            self.calib_result_label.setText("标定状态: ❌ 失败")
            self.calib_result_label.setStyleSheet("color: red; font-weight: bold;")
