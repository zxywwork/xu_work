...
# ============================================================
# 工作线程1：Vision Master 通信（仅用于手动触发，标定时自动断开）
# ============================================================
...
class RobotWorker(QThread):
    ...
    def _do_auto_calib(self, vision_idx):
        ...
        try:
            ...
            def calib_thread():
                try:
                    f, e, t = x5v.vision_auto_calib(self.handle, vision_idx)
                    flag[0] = f
                    errors[0] = e
                    transformation[0] = t
            ...
            thread = threading.Thread(target=calib_thread)
            thread.start()
            # 等待标定完成，超时120秒
            if not calib_done.wait(120):
                self.log_signal.emit("[标定] 超时（120秒），标定过程可能卡住，尝试终止")
                self.calib_result_signal.emit(-3, [], [])
                return
            ...
            e = errors[0]
            t = transformation[0]
            self.log_signal.emit(f"[标定] 返回标志 flag={f}")
            if f == 1:
                self.log_signal.emit(f"[标定] ✅ X方向平均误差: {e[0]:.4f} mm")
                self.log_signal.emit(f"[标定] ✅ Y方向平均误差: {e[1]:.4f} mm")
                self.log_signal.emit(f"[标定] ✅ X方向最大误差: {e[2]:.4f} mm")
                self.log_signal.emit(f"[标定] ✅ Y方向最大误差: {e[3]:.4f} mm")
            elif f == -1:
            ...
            self.calib_result_signal.emit(f, e, t)
        except Exception as ex:
            self.log_signal.emit(f"[标定] 异常: {str(ex)}")
            self.calib_result_signal.emit(-2, [], [])
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - 动态J2标定 v3 (带超时保护)")
    ...
    def on_apply_config(self):
        ...
        try:
            ...
            config = x5v.Vision(
                ...
                trigger_do=255,
                trigger_command=trigger_cmd.encode()
            )
    ...
    def on_start_calib(self):
        ...
        # 如果 Vision Master 连接着，先断开以避免干扰
        if self.is_vm_connected:
            self.append_log("[标定] 正在断开 Vision Master 连接，以避免冲突...")
            self.vm_thread.disconnect()
            # 等待断开完成，然后继续
            QTimer.singleShot(500, self._real_start_calib)
        else:
            self._real_start_calib()

    def _real_start_calib(self):
        # 使用同步 API 检查并准备机器人状态
        try:
            handle = self.robot_worker.handle
            if handle is None:
                self.append_log("[标定] 机器人句柄无效")
                return

            # 检查报警
            state = x5.get_system_state(handle)
            if state.alarm:
                self.append_log("[标定] 报警未清除，请先复位报警")
                self.calib_result_label.setText("标定状态: ❌ 报警未复位")
                return

            # 切换自动模式
            if state.mode != 2:
                self.append_log("[标定] 当前模式不是自动模式，正在切换...")
                x5.set_system_mode(handle, 2)
                time.sleep(0.5)
                state = x5.get_system_state(handle)
                if state.mode != 2:
                    self.append_log("[标定] 切换自动模式失败")
                    self.calib_result_label.setText("标定状态: ❌ 模式切换失败")
                    return

            # 上使能
            if not state.enable:
                self.append_log("[标定] 未上使能，正在上使能...")
                x5.enable_servo(handle, True)
                time.sleep(0.5)
                state = x5.get_system_state(handle)
                if not state.enable:
                    self.append_log("[标定] 上使能失败")
                    self.calib_result_label.setText("标定状态: ❌ 使能失败")
                    return

            # 停止运动
            if state.run != 3:
                self.append_log("[标定] 机器人正在运行，执行停止...")
                x5.abort(handle)
                time.sleep(0.5)
                state = x5.get_system_state(handle)
                if state.run != 3:
                    self.append_log("[标定] 停止运动失败")
                    self.calib_result_label.setText("标定状态: ❌ 停止失败")
                    return

            # 最终检查
            state = x5.get_system_state(handle)
            if state.mode != 2 or not state.enable or state.run != 3 or state.alarm:
                self.append_log("[标定] 机器人状态未就绪，请手动检查")
                self.calib_result_label.setText("标定状态: ❌ 状态异常")
                return

            self.append_log("[标定] ✅ 机器人状态已准备就绪 (自动模式, 上使能, 停止)")

        except Exception as e:
            self.append_log(f"[标定] 准备状态时异常: {e}")
            self.calib_result_label.setText("标定状态: ❌ 准备异常")
            return

        vision_idx = int(self.vision_index_combo.currentText().split()[-1])
        self.append_log("="*60)
        self.append_log("[标定] 开始自动标定流程...")
        self.append_log("[标定] 请确保标定板在相机视野内且视觉软件已就绪")
        self.append_log("="*60)
    ...
    def on_calib_result(self, flag, errors, transformation):
        if flag == 1:
            ...
            self.append_log("[标定] ⏰ 标定超时（120秒）")
            self.calib_result_label.setText("标定状态: ⏰ 超时")
        else:
            self.append_log("[标定] ❌ 标定失败")
            self.calib_result_label.setText("标定状态: ❌ 失败")

        # 标定完成后，如果之前断开了 Vision Master，尝试重新连接
        if not self.is_vm_connected:
            self.append_log("[标定] 正在重新连接 Vision Master...")
            self.vm_thread.connect_to_server()
