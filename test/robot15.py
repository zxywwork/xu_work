...
from PyQt5.QtCore import QThread, pyqtSignal, Qt, QTimer, QMetaObject, pyqtSlot
...
# ============================================================
# 工作线程2：机器人 API 调用（含MOVJ/MOVL点位运动）
# ============================================================
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - 完整版 (Z偏移可调+点可编辑)")
    ...
    def init_ui(self):
        ...
        self.movj_btn.clicked.connect(self.on_movj_test)
        ...
        pallet_inner.addWidget(QLabel("X"), 0, 2)
        pallet_inner.addWidget(self.p1_x_edit, 0, 3)
        pallet_inner.addWidget(QLabel("Y"), 0, 4)
        pallet_inner.addWidget(self.p1_y_edit, 0, 5)
        pallet_inner.addWidget(QLabel("Z"), 0, 6)
        pallet_inner.addWidget(self.p1_z_edit, 0, 7)
        pallet_inner.addWidget(QLabel("C"), 0, 8)
        pallet_inner.addWidget(self.p1_c_edit, 0, 9)

        # P2
        pallet_inner.addWidget(QLabel("P2 (PR11):"), 1, 0)
        self.p2_record_btn = QPushButton("记录P2")
        self.p2_record_btn.clicked.connect(lambda: self.record_pallet_point(11, "P2"))
        pallet_inner.addWidget(self.p2_record_btn, 1, 1)
        ...
        pallet_inner.addWidget(QLabel("X"), 1, 2)
        pallet_inner.addWidget(self.p2_x_edit, 1, 3)
        pallet_inner.addWidget(QLabel("Y"), 1, 4)
        pallet_inner.addWidget(self.p2_y_edit, 1, 5)
        pallet_inner.addWidget(QLabel("Z"), 1, 6)
        pallet_inner.addWidget(self.p2_z_edit, 1, 7)
        pallet_inner.addWidget(QLabel("C"), 1, 8)
        pallet_inner.addWidget(self.p2_c_edit, 1, 9)

        # P3
        pallet_inner.addWidget(QLabel("P3 (PR12):"), 2, 0)
        self.p3_record_btn = QPushButton("记录P3")
        self.p3_record_btn.clicked.connect(lambda: self.record_pallet_point(12, "P3"))
        pallet_inner.addWidget(self.p3_record_btn, 2, 1)
        ...
        pallet_inner.addWidget(QLabel("X"), 2, 2)
        pallet_inner.addWidget(self.p3_x_edit, 2, 3)
        pallet_inner.addWidget(QLabel("Y"), 2, 4)
        pallet_inner.addWidget(self.p3_y_edit, 2, 5)
        pallet_inner.addWidget(QLabel("Z"), 2, 6)
        pallet_inner.addWidget(self.p3_z_edit, 2, 7)
        pallet_inner.addWidget(QLabel("C"), 2, 8)
        pallet_inner.addWidget(self.p3_c_edit, 2, 9)

        # 更新PR按钮
        self.update_pr_btn = QPushButton("更新PR变量")
        self.update_pr_btn.clicked.connect(self.on_update_pr_from_edits)
        pallet_inner.addWidget(self.update_pr_btn, 3, 0, 1, 2)
        ...
        self.z_offset_edit = QLineEdit("50")  # 默认抬高到50
        ...
        self.approach_offset_edit = QLineEdit("50")
        ...
        pallet_inner.addWidget(self.point_table, 10, 0, 1, 6)

        # 进度显示
        pallet_inner.addWidget(QLabel("进度:"), 11, 0)
        self.progress_label = QLabel("0 / 0")
        pallet_inner.addWidget(self.progress_label, 11, 1)
        pallet_inner.addWidget(QLabel("当前目标:"), 11, 2)
        self.current_target_label = QLabel("--")
        pallet_inner.addWidget(self.current_target_label, 11, 3, 1, 3)

        # 重置
        self.reset_pallet_btn = QPushButton("重置码垛")
        self.reset_pallet_btn.clicked.connect(self.on_reset_pallet)
        pallet_inner.addWidget(self.reset_pallet_btn, 12, 0, 1, 2)

        # 提示
        pallet_inner.addWidget(QLabel("提示: 记录P1,P2,P3后可手动修改坐标，点击'更新PR变量'生效。Z基准偏移可整体抬高所有点。"), 13, 0, 1, 6)
        ...
        main_layout.addWidget(log_group, 1)  # 拉伸因子1，让日志占据剩余空间
    ...
    def _set_robot_controls_enabled(self, enabled):
        for w in [self.movj_btn, self.emergency_btn,
                  ...
                  self.update_pr_btn, self.reset_pallet_btn]:
    ...
    def on_movj_test(self):
        self.robot_worker.movj_to(0, 0, 0, 0, 0, 0)
    ...
    def record_pallet_point(self, pr_index, name):
        ...
        # 更新编辑框
        if name == "P1":
            self.p1_x_edit.setText(f"{p[0]:.3f}")
            self.p1_y_edit.setText(f"{p[1]:.3f}")
            self.p1_z_edit.setText(f"{p[2]:.3f}")
            self.p1_c_edit.setText(f"{p[5]:.3f}")
            self.p1_point = point_obj
        elif name == "P2":
            self.p2_x_edit.setText(f"{p[0]:.3f}")
            self.p2_y_edit.setText(f"{p[1]:.3f}")
            self.p2_z_edit.setText(f"{p[2]:.3f}")
            self.p2_c_edit.setText(f"{p[5]:.3f}")
            self.p2_point = point_obj
        elif name == "P3":
            self.p3_x_edit.setText(f"{p[0]:.3f}")
            self.p3_y_edit.setText(f"{p[1]:.3f}")
            self.p3_z_edit.setText(f"{p[2]:.3f}")
            self.p3_c_edit.setText(f"{p[5]:.3f}")
    ...
    def on_update_pr_from_edits(self):
        """从编辑框读取坐标，更新PR变量和内部point"""
        if not self.is_robot_connected:
            self.append_log("[错误] 请先连接机器人")
            return
        try:
            # P1
            x = float(self.p1_x_edit.text())
            y = float(self.p1_y_edit.text())
            z = float(self.p1_z_edit.text())
            c = float(self.p1_c_edit.text())
            pose = x5.Pose(x, y, z, 0, 0, c, 0, 0, 0)
            point = x5.Point(pose=pose, uf=self.pallet_uf, tf=self.pallet_tf, cfg=self.pallet_cfg)
            self.robot_worker.set_pr_value(10, point)
            self.p1_point = point
            # P2
            x = float(self.p2_x_edit.text())
            y = float(self.p2_y_edit.text())
            z = float(self.p2_z_edit.text())
            c = float(self.p2_c_edit.text())
            pose = x5.Pose(x, y, z, 0, 0, c, 0, 0, 0)
            point = x5.Point(pose=pose, uf=self.pallet_uf, tf=self.pallet_tf, cfg=self.pallet_cfg)
            self.robot_worker.set_pr_value(11, point)
            self.p2_point = point
            # P3
            x = float(self.p3_x_edit.text())
            y = float(self.p3_y_edit.text())
            z = float(self.p3_z_edit.text())
            c = float(self.p3_c_edit.text())
            pose = x5.Pose(x, y, z, 0, 0, c, 0, 0, 0)
            point = x5.Point(pose=pose, uf=self.pallet_uf, tf=self.pallet_tf, cfg=self.pallet_cfg)
            self.robot_worker.set_pr_value(12, point)
            self.p3_point = point
            self.append_log("[码垛] PR10, PR11, PR12 已从编辑框更新")
        except ValueError as e:
            self.append_log(f"[错误] 输入数值无效: {e}")
    ...
    def record_safe_point(self):
        ...
        self.safe_point_label.setText(f"X={p[0]:.1f} Y={p[1]:.1f} Z={p[2]:.1f}")
    ...
    def on_generate_pallet_points(self):
        ...
        row_vec = (p2.x - p1.x, p2.y - p1.y, 0)  # 忽略Z方向，让Z由偏移控制
        col_vec = (p3.x - p1.x, p3.y - p1.y, 0)

        if spacing_mode == 0:
            if row > 1:
                row_step = (row_vec[0] / (row - 1), row_vec[1] / (row - 1), 0)
            else:
                row_step = (0, 0, 0)
            if col > 1:
                col_step = (col_vec[0] / (col - 1), col_vec[1] / (col - 1), 0)
            else:
                col_step = (0, 0, 0)
        else:
            row_len = math.sqrt(row_vec[0]**2 + row_vec[1]**2)
            col_len = math.sqrt(col_vec[0]**2 + col_vec[1]**2)
            if row_len < 1e-6 or col_len < 1e-6:
                self.append_log("[错误] P2或P3与P1重合")
                return
            row_unit = (row_vec[0] / row_len, row_vec[1] / row_len, 0)
            col_unit = (col_vec[0] / col_len, col_vec[1] / col_len, 0)
            row_step = (row_unit[0] * row_dist, row_unit[1] * row_dist, 0)
            col_step = (col_unit[0] * col_dist, col_unit[1] * col_dist, 0)

        points = []
        for k in range(layer):
            z_layer = z_offset + k * height
            for i in range(row):
                for j in range(col):
        ...
        self.append_log(f"[码垛] 成功生成 {self.total_steps} 个目标点 (Z={z_offset:.1f}基准)")
