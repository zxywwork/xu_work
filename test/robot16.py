...
class RobotWorker(QThread):
    ...
    def __init__(self):
        ...
        self.cmd_lock = Lock()
        self._stop_flag = False  # 急停锁定：为 True 时丢弃所有运动类命令
    ...
    def abort_immediately(self, lock_motion=True):
        """立即停止机器人（不经过队列），并清空待执行命令。

        lock_motion=True : 进入急停锁定，后续运动命令会被丢弃，
                           直到“复位报警”或重新连接才解锁（用于真正的急停）。
        lock_motion=False: 仅停止当前动作并清空队列，不锁定（用于“停止运行”）。
        """
        with self.cmd_lock:
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                # 急停锁定期间：丢弃所有运动类命令（直到复位报警/重新连接解锁）
    ...
    def _do_connect(self):
        try:
            ...
            self._stop_flag = False  # 重新连接成功后解除急停锁定
            self.connected_signal.emit(True)
    ...
    def _do_reset_alarm(self):
        ...
        try:
            ...
            self._stop_flag = False  # 复位报警成功后解除急停锁定
    ...
    def _do_emergency_stop(self):
        ...
        try:
            x5.abort(self.handle)
            self.log_signal.emit("[急停] 已触发（机器人停止）")
        except Exception as e:
            self.log_signal.emit(f"[急停] 失败: {str(e)}")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        pallet_inner.addWidget(self.reset_pallet_btn, 8, 6)
        ...
        # 提示
        pallet_inner.addWidget(QLabel("提示: 记录P1/P2/P3或手动修改数值→应用修改→生成点→选择点或连续运行"), 12, 0, 1, 6)
    ...
    def _set_robot_controls_enabled(self, enabled):
        for w in [self.movj_btn, self.emergency_btn,
                  ...
                  self.safe_record_btn, self.apply_points_btn,
