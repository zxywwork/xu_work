...
class VisionMasterThread(QThread):
    ...
    def trigger_and_get(self):
        ...
        try:
            ...
            # self.sock.send((self.trigger_cmd + "\n").encode())
...
class RobotWorker(QThread):
    ...
    def get_uf(self, uf_index):
        if self.handle is None:
            return None, "机器人未连接"
        try:
            pose = x5.get_uf(self.handle, uf_index)
            return pose, None
        except Exception as e:
            return None, str(e)
    ...
    def run_auto_calib(self, vision_idx):
        ...
        try:
            ...
            if flag == 1:
                ...
                self.log_signal.emit("[标定] 可能原因:")
                self.log_signal.emit("  1. 标定过程中相机未识别到标定板")
                self.log_signal.emit("  2. 视觉数据格式不正确 (需要 X,Y,C,ATTR,ID;)")
                self.log_signal.emit("  3. 触发方式设置错误或未收到数据")
            else:
                self.log_signal.emit("[标定] ❌ 标定失败，请检查配置")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("视觉码垛控制系统 - SCARA J2 动态标定 v3")
        self.setGeometry(50, 50, 1200, 1050)
    ...
    def init_ui(self):
        ...
        # 用户坐标系查看
        robot_layout.addWidget(QLabel("查看UF:"))
        self.uf_view_combo = QComboBox()
        self.uf_view_combo.addItems([f"UF{i}" for i in range(17)])
        self.uf_view_combo.setCurrentIndex(15)
        self.uf_view_combo.setFixedWidth(80)
        robot_layout.addWidget(self.uf_view_combo)

        self.uf_view_btn = QPushButton("读取UF数据")
        self.uf_view_btn.clicked.connect(self.on_read_uf)
        self.uf_view_btn.setEnabled(False)
        robot_layout.addWidget(self.uf_view_btn)

        self.uf_display_label = QLabel("UF数据: --")
        self.uf_display_label.setMinimumWidth(300)
        robot_layout.addWidget(self.uf_display_label)
    ...
    def _set_robot_controls_enabled(self, enabled):
        for w in [self.movj_btn, self.movl_btn, self.emergency_btn,
                  ...
                  self.reset_alarm_btn, self.uf_view_btn]:
    ...
    def on_read_uf(self):
        ...
        uf_index = self.uf_view_combo.currentIndex()
        self.append_log(f"[UF] 正在读取 {self.uf_view_combo.currentText()} ...")

        pose, err = self.robot_worker.get_uf(uf_index)
        if err:
            self.append_log(f"[UF] 读取失败: {err}")
            self.uf_display_label.setText(f"UF{uf_index}: 读取失败")
            return

        self.uf_display_label.setText(
            f"UF{uf_index}: X={pose.x:.2f} Y={pose.y:.2f} Z={pose.z:.2f} "
            f"A={pose.a:.2f} B={pose.b:.2f} C={pose.c:.2f}"
        )
        self.append_log(f"[UF] UF{uf_index}: X={pose.x:.2f} Y={pose.y:.2f} Z={pose.z:.2f} "
                       f"A={pose.a:.2f} B={pose.b:.2f} C={pose.c:.2f}")
    ...
    def on_robot_disconnect(self):
        ...
        self._set_robot_controls_enabled(False)
        self.uf_display_label.setText("UF数据: --")
    ...
    # ============================================================
    # ★★★ 核心修正：写入配置（使用 _AutoDynamicNotEndData 实例）★★★
    # ============================================================
    def on_apply_config(self):
        ...
        try:
            ...
            self.append_log(f"[配置] 接收格式: {self.vm_format_combo.currentText()}")
            ...
            # ★★★ 核心修正：使用 _AutoDynamicNotEndData 实例 ★★★
            ...
            x5v.vision_set_config(self.robot_worker.handle, vision_idx, config)
            self.append_log("[配置] Vision 配置写入成功")

            self.append_log("[配置] 正在写入标定参数...")
            x5v.vision_write_calib_process(self.robot_worker.handle, vision_idx, calib_process)
            self.append_log("[配置] 标定参数写入成功")

            self.append_log("[配置] ✅ 配置全部写入成功")
    ...
    def on_diagnose(self):
        ...
        if 0 in attrs and 1 in attrs and 2 in attrs and 3 in attrs:
            self.append_log("[诊断] ✅ ATTR 包含 0,1,2,3")
        else:
            self.append_log("[诊断] ⚠️ ATTR 不完整")

        if self.base_point is None:
            self.append_log("[诊断] ❌ 未记录基准点")
            return
        self.append_log(f"[诊断] ✅ 基准点: {self.base_point}")

        self.append_log("[诊断] 读取控制器中的视觉配置...")
        try:
            configs = x5v.vision_get_config_list(self.robot_worker.handle)
            cfg = configs[0]
            self.append_log(f"[诊断] Vision 0: IP={cfg.ip.decode()}, 端口={cfg.port}, "
                          f"格式={cfg.data_format}, 安装类型={cfg.camera_mount_type}")

            calib = x5v.vision_read_calib_process(self.robot_worker.handle, 0)
            self.append_log(f"[诊断] 标定类型: {calib.calibration_type}")
            self.append_log(f"[诊断] 标定点数: {calib.point_count}")
            self.append_log(f"[诊断] 平面平行: {calib.is_workplane_parallel}")

            if hasattr(calib.calib_data, 'auto_dynamic_not_end'):
                data = calib.calib_data.auto_dynamic_not_end
                self.append_log(f"[诊断] ✅ 动态J2参数存在: tf_index={data.tf_index}, "
                              f"mark_distance={data.mark_distance}, step_size={data.step_size}")
            else:
                self.append_log("[诊断] ❌ auto_dynamic_not_end 不存在！")
        except Exception as e:
            self.append_log(f"[诊断] 读取配置失败: {str(e)}")
    ...
    def on_start_calib(self):
        ...
        if not self.latest_vm_points:
            self.append_log("[警告] 尚未触发拍照获取视觉数据")
            return

        attrs = set([p['attr'] for p in self.latest_vm_points])
        if 0 not in attrs or 1 not in attrs or 2 not in attrs or 3 not in attrs:
            self.append_log("[警告] ⚠️ ATTR 不全，标定可能失败！")
    ...
    def _do_start_calib(self):
        ...
        if flag == 1:
            ...
            self.calib_result_label.setText(f"标定状态: ✅ 成功 (X={avg_x:.3f}mm Y={avg_y:.3f}mm)")
            self.calib_result_label.setStyleSheet("color: green; font-weight: bold;")
            ...
            self.append_log("[标定] ❌ 标定失败 (flag={})".format(flag))
            self.calib_result_label.setText("标定状态: ❌ 失败")
            self.calib_result_label.setStyleSheet("color: red; font-weight: bold;")
