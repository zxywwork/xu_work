...
class RobotWorker(QThread):
    ...
    def wait_idle(self, timeout=20.0, target=None, tolerance=5.0, stop_check=None):
        """等待命令队列清空且机器人到位。
        ...
        - 返回 True 表示已到位，False 表示超时、被停止或连接断开。
        """
        start = time.time()
        ...
        def vision_pallet_thread():
            try:
                while self.vision_pallet_running and not self.vision_pallet_stop:
                    ...
                    if not self.robot_worker.wait_idle(timeout=10.0, target=photo_point.pose,
                                                       stop_check=lambda: self.vision_pallet_stop):
                        self.append_log("[视觉码垛] 未到达拍照点或等待超时，停止")
                        break
                    ...
                    for idx, pixel in enumerate(received_points):
                        ...
                        self.robot_worker.movl_to_pose(grab_pose, photo_point.uf,
                                                       photo_point.tf, photo_point.cfg)
                        if not self.robot_worker.wait_idle(timeout=10.0, target=grab_pose,
                                                           stop_check=lambda: self.vision_pallet_stop):
                            self.append_log("[视觉码垛] 未到达抓取点或等待超时，停止")
                            self.vision_pallet_stop = True
                            break
                        if self.vision_pallet_stop or not self.vision_pallet_running:
                            break
