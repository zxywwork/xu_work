...
职责：
  - 组装全部界面控件（机器人控制 / 视觉通信 / 实时状态 / 视觉标定 / 手动码垛）；
  - 编排各子模块：RobotWorker（机器人命令）、VisionMasterThread（视觉通信）、
    PalletPlanGenerator（码垛点生成）、vision_calib（标定构造/JSON）；
  - 事件槽跨线程安全：工作线程通过 pyqtSignal / QMetaObject.invokeMethod
    交回 GUI 线程更新界面。

线程安全约定（重要，来自原 robot20_sus.py 修复经验）：
  - 所有 x5/x5v SDK 调用都加 `with X5_LOCK:`，不要在持锁期间 sleep 或弹窗；
  - 子线程内禁止直接读 Qt 控件，启动线程前先把参数捕获成局部变量；
  - append_log 跨线程安全（QueuedConnection 投递到 GUI 线程执行）。
"""
import os
import time
import traceback
...
from PyQt5.QtCore import QThread, pyqtSlot, Qt, QTimer, QMetaObject, Q_ARG
from PyQt5.QtGui import QFont

import xapi.api as x5
import xapi.api.vision as x5v

from .common import (X5_LOCK, MODE_NAMES, PR_P1, PR_P2, PR_P3, PR_PHOTO, PR_SAFE,
                     LOG_DIR, CALIB_FILE_PATH, build_pose, build_point)
from .vision_master import VisionMasterThread
from .robot_worker import RobotWorker
from .pallet_logic import PalletPlanGenerator, make_approach_depart
from .vision_calib import (point_to_dict, point_from_dict,
                           build_config, build_calib_process,
                           save_calib, load_calib)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log_file_path = os.path.join(
            LOG_DIR, f"vision_pallet_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    def init_ui(self):
        ...
        self.p1_record_btn.clicked.connect(lambda: self.record_pallet_point(PR_P1, "P1"))
        ...
        self.p2_record_btn.clicked.connect(lambda: self.record_pallet_point(PR_P2, "P2"))
        ...
        self.p3_record_btn.clicked.connect(lambda: self.record_pallet_point(PR_P3, "P3"))
    ...
    def update_state(self, info):
        self.mode_label.setText(f"模式: {MODE_NAMES.get(info['mode'], str(info['mode']))}")
    ...
    def on_apply_config(self):
        ...
        try:
            ...
            calib_process = build_calib_process(params)
    ...
    def _collect_calib_params(self, vision_idx):
        ...
        return {
            ...
            'base_point': point_to_dict(self.base_point),
        }
    ...
    def _apply_calib_params_to_ui(self, params):
        ...
        if bp:
            self.base_point = point_from_dict(bp)
            self.base_point_label.setText(
    ...
    def on_save_calib(self):
        ...
        try:
            params = self._collect_calib_params(vision_idx)
            save_calib(CALIB_FILE_PATH, vision_idx, params, self.calib_data[vision_idx])
            self.append_log(f"[标定] ✅ 视觉工艺 {vision_idx} 标定结果已保存: {CALIB_FILE_PATH}")
    ...
    def on_load_calib(self):
        ...
        if not os.path.exists(CALIB_FILE_PATH):
            QMessageBox.warning(self, "提示", f"未找到标定文件: {CALIB_FILE_PATH}")
            return
        try:
            data = load_calib(CALIB_FILE_PATH)
        except Exception as e:
    ...
    def on_verify(self):
        ...
        trig_point = x5.Point(pose=trig_pose, uf=pose[6], tf=pose[7],
                              cfg=pose[8] if len(pose) > 8 else (0, 0, 0, 1))
    ...
    def record_pallet_point(self, pr_index, name):
        ...
        cfg = pose[8] if len(pose) > 8 else (0, 0, 0, 1)
        p = pose[:6]
        point_obj = build_point(build_pose(*p), uf, tf, cfg)
    ...
    def apply_point_modifications(self):
        ...
        self.p1_point = build_point(build_pose(p1_x, p1_y, p1_z, 0, 0, p1_c), uf, tf, cfg)
        self.robot_worker.set_pr_value(PR_P1, self.p1_point)

        self.p2_point = build_point(build_pose(p2_x, p2_y, p2_z, 0, 0, p2_c), uf, tf, cfg)
        self.robot_worker.set_pr_value(PR_P2, self.p2_point)

        self.p3_point = build_point(build_pose(p3_x, p3_y, p3_z, 0, 0, p3_c), uf, tf, cfg)
        self.robot_worker.set_pr_value(PR_P3, self.p3_point)
    ...
    def record_safe_point(self):
        ...
        cfg = pose[8] if len(pose) > 8 else (0, 0, 0, 1)
        p = pose[:6]
        self.safe_point = build_point(build_pose(*p), uf, tf, cfg)
        self.robot_worker.set_pr_value(PR_SAFE, self.safe_point)
    ...
    def move_to_safe_point(self):
        ...
        if move_type == "MOVJ":
            self.robot_worker.movj_to_pose(self.safe_point.pose, self.safe_point.uf,
                                           self.safe_point.tf, self.safe_point.cfg)
        else:
            self.robot_worker.movl_to_pose(self.safe_point.pose, self.safe_point.uf,
                                           self.safe_point.tf, self.safe_point.cfg)
    ...
    def record_photo_point(self):
        ...
        cfg = pose[8] if len(pose) > 8 else (0, 0, 0, 1)
        p = pose[:6]
        self.photo_point = build_point(build_pose(*p), uf, tf, cfg)
        self.robot_worker.set_pr_value(PR_PHOTO, self.photo_point)
    ...
    def move_to_photo_point(self):
        ...
        move_type = self.move_type_combo.currentText()
        if move_type == "MOVJ":
            self.robot_worker.movj_to_pose(self.photo_point.pose, self.photo_point.uf,
                                           self.photo_point.tf, self.photo_point.cfg)
        else:
            self.robot_worker.movl_to_pose(self.photo_point.pose, self.photo_point.uf,
                                           self.photo_point.tf, self.photo_point.cfg)
    ...
    def on_generate_pallet_points(self):
        ...
        try:
            points = PalletPlanGenerator.generate(
                p1, p2, p3, row, col, layer, height, z_offset,
                spacing_mode, row_dist, col_dist)
        except ValueError as e:
    ...
    def _move_to_point(self, idx, move_type=None, approach_offset=None, depart_offset=None, points=None):
        ...
        approach_pose, depart_pose = make_approach_depart(target_pose, approach_offset, depart_offset)
            ...
            try:
                while self.vision_pallet_running and not self.vision_pallet_stop:
                    ...
                    for idx, pixel in enumerate(received_points):
                        ...
                        approach_pose, depart_pose = make_approach_depart(
                            target_pose, approach_offset, depart_offset)

                        if move_type == "MOVJ":
                            self.robot_worker.movj_to_pose(approach_pose, pallet_uf, pallet_tf, pallet_cfg)
                            self.robot_worker.movj_to_pose(target_pose, pallet_uf, pallet_tf, pallet_cfg)
                            self.robot_worker.movj_to_pose(depart_pose, pallet_uf, pallet_tf, pallet_cfg)
                        else:
                            self.robot_worker.movl_to_pose(approach_pose, pallet_uf, pallet_tf, pallet_cfg)
                            self.robot_worker.movl_to_pose(target_pose, pallet_uf, pallet_tf, pallet_cfg)
                            self.robot_worker.movl_to_pose(depart_pose, pallet_uf, pallet_tf, pallet_cfg)
